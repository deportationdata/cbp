...
contained_pairs_path <- file.path(
  validation_dir,
  "apprehensions-contained-pairs.parquet"
)

daily_comparison_path <- file.path(
  validation_dir,
  "apprehensions-overlap-daily-comparison.parquet"
)

pair_summary_path <- file.path(
  validation_dir,
  "apprehensions-overlap-pair-summary.parquet"
)
...
#### Find Contained Pairs #### 

# find contained pairs (files contained in others are considered smaller)
contained_pairs <- parts_metadata |>
  ...
  filter(
    ...
    # larger starts before smaller
    larger_min_date <= smaller_min_date,
    # larger ends after smaller
    larger_max_date >= smaller_max_date,
    # larger starts before or ends after or comes before alphabetically
    larger_min_date < smaller_min_date |
      larger_max_date > smaller_max_date |
      smaller_file < larger_file
  ) |>
