...
#### Exact Matches? #### 
# read same-date pairs
same_date <- read_parquet(same_date_ranges_path)

# check if col contains at least one nonblank value
has_data <- function(x) {
  any(
    !is.na(x) &
...
# compare each same-date pair
for (pair_number in seq_len(nrow(same_date))) {
  ...
  # extract file names
  files <- str_split(
    same_date$files_and_rows[pair_number],
    " \\| "
  )[[1]] |>
    str_remove(" \\[rows:.*")
  ...
  # skip pair if at least one part already removed
  if (!file_exists(file_a) || !file_exists(file_b)) {
    
    cat(
      "Skipping pair",
      pair_number,
      ": One or both parts are no longer in parts_to_stack.\n"
    )
  ...
  column_differences <- map_dfr(
    ...
    function(col) {
      ...
      vals_a <- unique(
        na.omit(
          as.character(rows_only_in_a[[col]])
        )
      )
      
      vals_b <- unique(
        na.omit(
          as.character(rows_only_in_b[[col]])
        )
      )
