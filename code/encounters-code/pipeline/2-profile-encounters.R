...
# outputs
sheet_inventory_path <- file.path(metadata_dir, "sheet-inventory.parquet")
...
if (!force_reprofile) {
  ...
  if (file.exists(raw_column_inventory_path)) {
    old_column_inventory <- read_parquet(raw_column_inventory_path)
  }
...
raw_column_inventory <- bind_rows(
...
distinct_columns <- raw_column_inventory |>
